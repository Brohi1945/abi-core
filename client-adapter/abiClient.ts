...
// Customer-facing variant (Phase 2) — abos-chat ke customer widget se
// use hoga. Alag endpoint, alag (chota, safe) tool set, alag persona —
// admin ke askABI() se koi lena dena nahi.
export async function askABICustomer(
  command: string,
  authToken: string,
  sourceApp: "abos-chat" | string,
  history: Array<{ role: string; content: string }> = [],
  ...
  const res = await fetch(`${apiUrl}/api/customer-command`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${authToken}`,
    },
    body: JSON.stringify({ command, sourceApp, history, conversation_id: conversationId }),
  });
